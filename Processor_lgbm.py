...
class Preprocessor:
    ...
    def preprocessing(self, df):
        ...
        df['DI'] = self.DI(df['temperature'], df['humidity'])
        ...
        for num in self.clust_to_num[1]:
            temp_df = dfs[num - 1]
            temp_df['working_time'] = (
                        (temp_df['hour'].isin(range(8, 20))) & (temp_df['weekday'].isin([0, 1, 2, 3, 4]))).astype(int)
            temp_df['lunch_time'] = (
